...
exports.all = function (req, res) {
    ...
    whereClause(req,res,filters, currentUser, function (err, clause,q,limit,offset,al,status) {
        ...
        if(status == false){
            ...
            db.lead_doc_mapping.findAndCountAll({
                where : q || null,
                limit: limit , offset: offset,
                order: [["last_updated","ASC"]]
            }).then(function(o){

                var items = o.rows.map(function(item){
                    return item.toModel();
                });
                var moreRecordsAvailable = false;

                if(o.count > o.rows.length)
                {
...
exports.create = function(req,res){
    ...
    db.lead_doc_mapping.create(toEntity(data,req,res,"create",currentTime)).then(function(i){
...
var toEntity = function(data,req,res,action,currentTime){
    ...
    var json = {
        ...
        lead_id: data.LeadId,
        type: data.Type,
        thumbnail: base64ToBlob(data.Thumbnail),
        s3_key: data.S3Key,
        s3_id: data.S3Id,
        created_on: data.CreatedOn,
        doc_desc: data.DocDesc,
    ...
    if(action == "create"){
        json.initial_create = currentTime;
    }
...
var base64ToBlob = function(base64String){
    ...
    var blob = new Buffer (base64String,'base64').toString('binary');
    return blob;
            ...
            reportingPerson.all(req,res,currentUser.EmployeeId,function(empObject){
                ...
                if(filters.timestamp && filters.emp_id)
                {
                    ...
                    if(empArray.indexOf(empId) != -1)
                    {
                        ...
                        q = { employee_id : filters.emp_id , last_updated : { $gt : filters.timestamp },active:1 };
                ...
                }else
                {
                    ...
                    q = { employee_id : currentUser.EmployeeId , active: 1 };
                    if(filters.timestamp)
                    {
                        q = { employee_id : currentUser.EmployeeId,last_updated : { $gt : filters.timestamp }, active: 1  };
                    }
        ...
        }else if(al && al.access_level == 1)
        {
            ...
            if(filters.timestamp && filters.emp_id)
            {
                ...
                if(currentUser.EmployeeId == filters.emp_id)
                {
                    ...
                    q = { employee_id : filters.emp_id , last_updated : { $gt : filters.timestamp }, active: 1  };
                    callback(null, clause,q,limit,offset,al,1);
