   ...
   db.employee.findAll().then(function(emp){
            ...
            async.forEachSeries(emp,function(empObject,call_1)
            {
               ...
               async.forEachSeries(allTables,function(table_name,call_2)
               {
                 ...
                 async.forEachSeries(period,function(pd,call_3)
                 {
                    ...
                    db_pr[table_name].count(
                    {
                      ...
                      include: [{ model: db_pr.lead_stage_calculation,include: [{model: db_pr.lead_stage_settings, where:{ position_level: 1}}]},{ model: db.leadsource, where:{ name: 'Referral' } }],
                      
                    }).then(function(c){
                         console.log(table_name+ "  Records are  for "+empObject.employee_id+"  is "+c +"  for---"+pd[0]);
                         db.new_referral_lead.create({ employee_id: empObject.employee_id , no_of_records: c ,type:pd[0],table_name: table_name,start_time:pd[1],end_time: pd[2],entry_on: currentTime }).then(function(up){
                          call_3();
                         });
               ...
               if(err) return next(err);
                ...
                console.log("New Referral Lead executed !");
